...
def cifar10_loader(batch_size, num_workers, datapath, cuda=False,image_size=32):
...
def cifar100_loader(batch_size, num_workers, datapath, cuda=False, image_size=32):
    normalize = transforms.Normalize(
        mean=[0.4914, 0.4822, 0.4465],
        std=[0.2023, 0.1994, 0.2010])
    transform_train = transforms.Compose([
...
def imagenet_loader(batch_size, num_workers, datapath, cuda=False, image_size=224):
...
def DataLoader(batch_size, dataset='cifar10', num_workers=4, datapath='../data', cuda=True):
